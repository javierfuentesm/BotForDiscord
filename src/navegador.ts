...
export const informe = async () => {
  ...
  const buildJenkins = await page.textContent(
    "#content > div > div.app__content > div > div:nth-child(2) > div:nth-child(3) > div.widget__body > div > div > span > div.table__col.table__col_right.executor > a"
  );
  await page.click(
  ...
  await page.close();
  await browser.close();
  return [casos, casosTotal, elements, buildJenkins?.trim()];
};
